...
mod aesgcm_tests {
    ...
    fn try_decrypt_b64(
        ...
        let priv_key_raw = crate::common::b64_decode_url(priv_key)?;
        let pub_key_raw = crate::common::b64_decode_url(pub_key)?;

        let ec_key = EcKeyComponents::new(priv_key_raw, pub_key_raw);
        let auth_secret = crate::common::b64_decode_url(auth_secret)?;
    ...
    fn test_decode() {
        ...
        let input ="8Vyes671P_VDf3G2e6MgY6IaaydgR-vODZZ7L0ZHbpCJNVaf_2omEms2tiPJiU22L3BoECKJixiOxihcsxWMjTgAcplbvfu1g6LWeP4j8dMAzJionWs7OOLif6jBKN6LGm4EUw9e26EBv9hNhi87-HaEGbfBMGcLvm1bql1F";
        let ciphertext = crate::common::b64_decode_url(input).unwrap();
        ...
        let block = AesGcmEncryptedBlock::new(
            &crate::common::b64_decode_url(dh).unwrap(),
            &crate::common::b64_decode_url(salt).unwrap(),
            4096,
            ciphertext,
        )
        .unwrap();
    ...
    fn test_decode_padding() {
        ...
        // Incoming Body (this is normally raw bytes. It's encoded here for presentation)
        let input ="Ua3-WW5kTbt11dBTiXBP6_hLBYhBNOtDFfue5QHMTd2DicL0wutDnt5z9pjRJ76w562egPq5qro95YLnsX0NWGmDQbsQ0Azds6jcBGsxHPt0p5GELAtR4AJj2OsB_LV7dTuGHN2SqsyXLARjTFN2wsF3xWhmuw";
        let ciphertext = crate::common::b64_decode_url(input).unwrap();
